...
Existen tantos autovalores como número de filas (m) o columnas (n) tiene la matriz original.
...
p(λ)=\|A–λI\| es un polinomio de grado n dependiente de λ --\> **polinomio característico de la matriz** A**A**  
...
Visualizando el ejemplo de la matriz A. Se observa que se cumple la igualdad A x = λ x. Además, se ve como AX1 = X1 \* un escalar (lambda 1) y lo mismo para X2.

Además, cuando el autovalor es positivo, el autovector se expande en la misma dirección mientras que cuando el autovalor es negativo el autovalor se expande en la dirección opuesta.
